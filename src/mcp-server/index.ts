...
async function handleGetRecommendations(args: any) {
  ...
  const output = await orchestrator.generateDocumentation(projectPath, {
    selectiveAgents: ['recommendation-engine'],
    userPrompt: `Provide ${focusArea || 'comprehensive'} recommendations`,
    agentOptions: {
      searchMode: 'keyword',
    },
  });

  const recSection = output.customSections.get('recommendation-engine') as any;
  ...
  return {
    content: [
      {
        type: 'text',
        text: recSection.content || recSection.markdown || 'No recommendation data available',
      },
    ],
  };
}
