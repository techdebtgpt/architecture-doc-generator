...
Generate the enhancement section now:`;
    ...
    // First, ask LLM to decide: update existing file or create new one?
    const routingPrompt = `Given this user focus area: "${userPrompt}"

And these existing documentation files:
${Object.keys(existingDocs)
  .map((file) => `- ${file}`)
  .join('\n')}

Should we:
A) UPDATE an existing file (if the topic closely matches an existing document)
B) CREATE a new file (if the topic is distinct and doesn't fit existing files)

Respond in this format:
DECISION: UPDATE or CREATE
TARGET_FILE: [filename.md if UPDATE, or suggested-filename.md if CREATE]
REASON: [brief explanation]`;

    const routingDecision = await model.invoke(routingPrompt);
    const routingText =
      typeof routingDecision === 'string'
        ? routingDecision
        : routingDecision.content?.toString() || '';

    const isUpdate = /DECISION:\s*UPDATE/i.test(routingText);
    const targetFileMatch = routingText.match(/TARGET_FILE:\s*(.+\.md)/i);
    const targetFile = targetFileMatch ? targetFileMatch[1].trim() : null;

    this.logger.info(`Decision: ${isUpdate ? 'UPDATE' : 'CREATE'} - ${targetFile || 'new file'}`);
    ...
    const executionTime = Date.now() - startTime;
    let enhancementFileName: string;
    let enhancementFilePath: string;
    let finalContent: string;

    if (isUpdate && targetFile && existingDocs[targetFile]) {
      // UPDATE MODE: Append to existing file
      enhancementFileName = targetFile;
      enhancementFilePath = path.join(existingDocsPath, enhancementFileName);

      const existingContent = existingDocs[targetFile];
      finalContent = `${existingContent}

---

## Enhancement: ${userPrompt}
...
${enhancementText}`;
      ...
      await fs.writeFile(enhancementFilePath, finalContent, 'utf-8');
      this.logger.info(`✅ Updated existing file: ${enhancementFileName}`);
    } else {
      // CREATE MODE: New file with topic-based name
      const topicSlug = userPrompt
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
        .substring(0, 50);

      enhancementFileName = targetFile || `${topicSlug}.md`;
      enhancementFilePath = path.join(existingDocsPath, enhancementFileName);
...
*This section was generated as an incremental enhancement to existing documentation.*`;
      ...
      await fs.writeFile(enhancementFilePath, finalContent, 'utf-8');
      this.logger.info(`✅ Created new file: ${enhancementFileName}`);
    }

    // Update or create changelog.md with versioning information
    await this.updateChangelog(existingDocsPath, {
      timestamp: new Date(),
      type: 'enhancement',
      prompt: userPrompt,
