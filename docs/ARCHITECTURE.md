...
- 🔴 **Critical** - Immediate action required (hardcoded secrets, SQL injection)
- 🟠 **High** - Important to fix soon (weak crypto, missing auth)
- 🟡 **Medium** - Should address (missing security headers, weak validation)
- 🟢 **Low** - Good to improve (outdated libraries, minor issues)
- 🔵 **Info** - Recommendations (security best practices)
...
```typescript
export class SecurityAnalyzerAgent implements Agent {
  async execute(context: AgentContext, options?: AgentExecutionOptions): Promise<AgentResult> {
    const findings = await this.analyzeSecurityPatterns(context);
    ...
    return {
      ...
      markdown: this.formatSecurityReport(findings),
      confidence: 0.9,
    };
  }
}
...
All agents inherit from `BaseAgentWorkflow` which provides a Pino logger instance:
...
export abstract class BaseAgentWorkflow {
  ...
  constructor() {
    this.logger = createLogger({ context: this.constructor.name });
  }
}
```
...
1. **Never use `_error` prefix** - All caught errors should be logged with proper context
2. **Provide context** - Include relevant data (file paths, inputs, partial outputs)
3. **Type-safe error messages** - Use `error instanceof Error ? error.message : String(error)`
4. **Choose appropriate level**:
   - `debug` for expected failures (file not found, missing config)
   - `warn` for unexpected but recoverable failures (parsing errors)
   - `error` for critical failures (agent crashes, unrecoverable errors)
5. **Silent fallbacks only when acceptable** - TokenManager uses silent fallbacks for token estimation

### Example: File Reading with Proper Error Logging

```typescript
private async readSourceFiles(files: string[]): Promise<Map<string, string>> {
...
### Example: LLM Output Parsing with Error Recovery

```typescript
private async parseAnalysisResult(llmOutput: string): Promise<AnalysisResult> {
  try {
    const parsed = JSON.parse(llmOutput);
    return AnalysisResultSchema.parse(parsed);
  } catch (error) {
    this.logger.warn('Failed to parse LLM output, using fallback', {
      error: error instanceof Error ? error.message : String(error),
      output: llmOutput.substring(0, 200),
    });
