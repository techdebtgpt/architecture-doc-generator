...
All agents must follow consistent error handling patterns:
...
try {
  ...
  this.logger.debug('Skipping unreadable file', {
...
4. **Choose appropriate logging level**:
   - `debug` for expected failures (file not found, missing dependencies)
   - `warn` for recoverable failures (LLM parsing errors)
   - `error` for critical failures (agent crashes)
5. **Silent fallbacks only when acceptable** - TokenManager estimation is an exception
