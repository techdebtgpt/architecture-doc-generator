...
```json
{
  ...
  "agents": {
    "enabled": ["file-structure", "dependency-analyzer", "pattern-detector", "security-analyzer"],
    "parallel": true
  },
  "refinement": {
    "enabled": true,
...
└── security.md               # Security analysis (NEW!)
```
...
# Security audit (NEW!)
archdoc analyze --prompt "analyze security vulnerabilities and authentication patterns"
```
...
archdoc analyze --agents dependency-analyzer,security-analyzer

# Full analysis with all 7 agents
archdoc analyze --agents file-structure,dependency-analyzer,pattern-detector,flow-visualization,schema-generator,architecture-analyzer,security-analyzer
```
